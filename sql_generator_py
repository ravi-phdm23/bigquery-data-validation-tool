...
def convert_business_logic_to_safe_sql(derivation_logic, source_table, project_id, dataset_id, column_manager=None):
    ...
    try:
        ...
        # String concatenation
            ...
            # Parse the CONCAT logic to extract requested fields
            import re
            concat_match = re.search(r'CONCAT\((.*?)\)', logic, re.IGNORECASE)
            if concat_match:
                concat_content = concat_match.group(1)
                # Check if it's a name concatenation pattern
                if 'first_name' in concat_content and 'last_name' in concat_content:
                    # Use configurable name concatenation
                    return column_manager.get_name_concat_expression(source_table)
                elif 'address' in concat_content or 'email' in concat_content:
                    # Handle address concatenation
                    parts = []
                    for part in concat_content.split(','):
                        part = part.strip().strip('"\'')
                        if part in ['first_name', 'last_name', 'address', 'email']:
                            mapped_col = column_manager.map_column(source_table, part)
                            if column_manager.has_column(source_table, mapped_col):
                                parts.append(mapped_col)
            ...
            # Fallback to configured name concatenation
            return column_manager.get_name_concat_expression(source_table)
