...
def execute_all_excel_scenarios():
    ...
    if not project_id:
        st.error("❌ **Google Cloud Project ID not configured!**")
        st.error("**Table is not accessible.** Please configure your project ID in the settings.")
        st.info("💡 Go to Settings → Set your Google Cloud Project ID in the sidebar")
        return
        ...
        try:
            ...
            if sql_query:
                ...
                if query_result and query_result['status'] == 'success':
                    ...
                    st.write(f"❌ Error executing query for scenario {scenario['scenario_name']}: {error_msg}")
                    results.append({
                        'scenario_name': scenario['scenario_name'],
                        'status': 'ERROR',
                        'total_rows': 0,
...
def generate_sql_for_scenario(scenario: Dict[str, Any], project_id: str = None, dataset_id: str = 'banking_sample_data') -> str:
    ...
    if not project_id:
        return """-- ❌ ERROR: Google Cloud Project ID not configured!
-- Table is not accessible. Please configure your project ID in the settings.
-- Go to Settings → Set your Google Cloud Project ID in the sidebar
...
SELECT 'SQL generation error' as error_message;"""
